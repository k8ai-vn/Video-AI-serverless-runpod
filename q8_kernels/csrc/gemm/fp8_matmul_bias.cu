...
__global__ void fp8_kernel_bias(const float_e4m3_t * Aptr, const float_e4m3_t * Bptr, const float* bias_ptr,
    ...
    float_e4m3_t *Ashm = (float_e4m3_t*)(shm_data + cosize(SmemLayoutScaleB{}) + cosize(SmemLayoutScaleA{}) + cosize(SmemLayoutScaleB{}));
...
void run_fp8_gemm_bias(void *A, void *B,  float* bias, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream){
    ...
    BOOL_SWITCH(fuse_gelu, fuse_gelu_, 
        BOOL_SWITCH(is_even, Is_Even, 
            BATCH_SWITCH(BA, BB, auto kernel = &fp8_kernel_bias<Is_Even, fuse_gelu_, BM, BN, BK, BA_, BB_, KStages, output_t, MMA,
                                                G2SCopyA, G2SCopyB, 
                ...
                kernel<<<grid, block, shm_size, stream>>>((float_e4m3_t*)A, (float_e4m3_t*)B, bias, A_scales, B_scales, (output_t*)C, M, N, K, BATCH););
        );
    );
}
