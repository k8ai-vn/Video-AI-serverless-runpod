...
// __forceinline__ __device__ auto convert_type_fp16(Tensor<Engine, Layout> const &tensor) {
...
__global__ void gemm_fp8_kernel(const float_e4m3_t * Aptr, const float_e4m3_t * Bptr, 
    ...
    output_t* Cshm = (output_t*)(shm_data + cosize(SmemLayoutScaleA{}) + cosize(SmemLayoutScaleA{}));

    float_e4m3_t *Ashm = (float_e4m3_t*)(shm_data + cosize(SmemLayoutScaleA{}) + cosize(SmemLayoutScaleA{}));
    float_e4m3_t *Bshm = (float_e4m3_t*)(Ashm + cosize(SmemLayoutA{}));
    ...
    auto tCrD_fp32 = thr_mma.partition_fragment_C(gD);           
    clear(tCrD_fp32);
...
#pragma unroll
    ...
    for (int itile = 0; itile < ntile; ++itile) {
        ...
        for (int ik = 0; ik < nk; ++ik) {
            ...
            cute::gemm(tiled_mma, tCrD_fp32, tCrA(_, _, ik), tCrB(_, _, ik), tCrD_fp32);
        }  // for ik
    }
    __syncthreads();
        ...
        for (size_t k = 0; k < global_k; k++)
        {
            ...
            {   
                if constexpr (std::is_same<output_t, float_e4m3_t>::value){
                    ...
                    auto fragment = convert_type_fp16<cute::bfloat16_t>(tCrC_r2s(_, i, t+k*repeat_k));
...
void run_fp8_gemm(void *A, void *B, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream){
    ...
    using mma_op = SM89_16x8x32_F32F8F8F32_E4M3_TN;
    ...
    using g2s_copy_atom = Copy_Atom<g2s_copy_traits, float_e4m3_t>;
    using G2SCopyA =
    ...
    using s2r_copy_atom_a = Copy_Atom<s2r_copy_traits_a, float_e4m3_t>;

    using s2r_copy_op_b = SM75_U32x2_LDSM_N;
    using s2r_copy_traits_b = Copy_Traits<s2r_copy_op_b>;
    using s2r_copy_atom_b = Copy_Atom<s2r_copy_traits_b, float_e4m3_t>;
    ...
    static constexpr int kShmSize = cute::max(shm_size_AB, shm_size_C*sizeof(output_t)) + shm_size_scales*sizeof(float);

    int shm_size = kShmSize;
    bool is_even = M % BM == 0;
    BOOL_SWITCH(fuse_gelu, fuse_gelu_, 
        BOOL_SWITCH(is_even, Is_Even,
            BATCH_SWITCH(BA, BB, auto kernel = &gemm_fp8_kernel<Is_Even, fuse_gelu_, BM, BN, BK, BA_, BB_, KStages, output_t,
                                                MMA,
                                                G2SCopyA, G2SCopyB, 
                ...
                kernel<<<grid, block, shm_size, stream>>>((float_e4m3_t*)A, (float_e4m3_t*)B, A_scales, B_scales, (output_t*)C, M, N, K, BATCH););
        );
    );
}

template void run_fp8_gemm<cutlass::float_e4m3_t>(void *A, void *B, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream);
template void run_fp8_gemm<cutlass::bfloat16_t>(void *A, void *B, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream);
