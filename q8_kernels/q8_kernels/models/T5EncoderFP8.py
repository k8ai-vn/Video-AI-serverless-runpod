...
class T5DenseActDense(nn.Module):
    def __init__(self, config: T5Config):
        super().__init__()
        self.wi = FP8Linear(config.d_model, config.d_ff, bias=False)
        self.wo = FP8Linear(config.d_ff, config.d_model, bias=False)
...
class T5DenseGatedActDense(nn.Module):
    def __init__(self, config: T5Config):
        super().__init__()
        self.wi_0 = FP8Linear(config.d_model, config.d_ff, bias=False)
        self.wi_1 = FP8Linear(config.d_model, config.d_ff, bias=False)
        self.wo = FP8Linear(config.d_ff, config.d_model, bias=False)
        self.dropout = nn.Dropout(config.dropout_rate)
        self.act = ACT2FN[config.dense_act_fn]
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.q = FP8Linear(self.d_model, self.inner_dim, bias=False)
        self.k = FP8Linear(self.d_model, self.inner_dim, bias=False)
        self.v = FP8Linear(self.d_model, self.inner_dim, bias=False)
        self.o = FP8Linear(self.inner_dim, self.d_model, bias=False)
...
class T5ClassificationHead(nn.Module):
    ...
    def __init__(self, config: T5Config):
        super().__init__()
        self.dense = FP8Linear(config.d_model, config.d_model)
        self.dropout = nn.Dropout(p=config.classifier_dropout)
        self.out_proj = FP8Linear(config.d_model, config.num_labels)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            module,
            (T5Model, T5ForConditionalGeneration, T5EncoderModelFP8, T5ForQuestionAnswering),
        ):
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.lm_head = FP8Linear(config.d_model, config.vocab_size, bias=False)
...
class T5EncoderModelFP8(T5PreTrainedModel):
...
class T5ForTokenClassification(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.classifier = FP8Linear(config.hidden_size, config.num_labels)
...
class T5ForQuestionAnswering(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.num_labels = config.num_labels
        self.qa_outputs = FP8Linear(config.hidden_size, config.num_labels)
