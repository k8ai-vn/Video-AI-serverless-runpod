...
void run_q8_gemm(int8_t *A, int8_t *B, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream){
    ...
    BOOL_SWITCH(fuse_gelu, fuse_gelu_, 
        BOOL_SWITCH(is_even, Is_Even,
            BATCH_SWITCH(BA, BB, auto kernel = &gemm_q8_kernel<Is_Even, fuse_gelu_, BM, BN, BK, BA_, BB_, KStages, output_t,
                                                MMA,
                                                G2SCopyA, G2SCopyB, 
                ...
                kernel<<<grid, block, shm_size, stream>>>((int8_t*)A, (int8_t*)B, A_scales, B_scales, (output_t*)C, M, N, K, BATCH););
        );
    );
}

template void run_q8_gemm<cutlass::float_e4m3_t>(int8_t *A, int8_t *B, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream);
template void run_q8_gemm<cutlass::bfloat16_t>(int8_t *A, int8_t *B, void *C, float* A_scales, float* B_scales, int BA, int BB, int M, int N, int K, bool fuse_gelu, cudaStream_t stream);
